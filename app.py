...
# Optional: Use python-dotenv or Streamlit secrets
...
def kpi_card(label: str, value: float, suffix: str = "", help_text: str = ""):
    ...
    with col:
        st.markdown(f"**{label}**")
        st.markdown(f"<span style='font-size:28px;font-weight:700'>{value:,.2f}{suffix}</span>", unsafe_allow_html=True)
        if help_text:
...
with c1: kpi_card("Avg Uptime", kpis["uptime_avg"], suffix="%")
with c2: kpi_card("Tickets Resolved (30d)", kpis["tickets_resolved_sum"], help_text="Total in the last 30 days")
with c3: kpi_card("Avg Resolution Time", kpis["avg_resolution_hrs"], suffix=" hrs")
with c4: 
    mom = kpis["cost_mom_change"]
    lbl = f"IT Cost (MoM)"
    if pd.notna(mom):
        kpi_card(lbl, mom, suffix=" %", help_text=f"Last month avg: ${kpis['cost_last']:,.0f}")
    else:
        kpi_card(lbl, np.nan, suffix=" %", help_text="Need at least 2 months of data")
